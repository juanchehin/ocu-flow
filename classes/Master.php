...
Class Master extends DBConnection {
	...
	function save_category(){
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "Categoria already exists.";
			return json_encode($resp);
			exit;
		}
		if(empty($id)){
		...
		if($save){
			...
			$resp['status'] = 'success';
			if(empty($id))
				$resp['msg'] = "New Categoria successfully saved.";
			else
				$resp['msg'] = " Categoria successfully updated.";
		}else{
			$resp['status'] = 'failed';
	...
	function delete_category(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Categoria successfully deleted.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_product(){
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "Producto already exists.";
			return json_encode($resp);
			exit;
		}
		if(empty($id)){
		...
		if($save){
			...
			$resp['status'] = 'success';
			if(empty($id)){
				$resp['msg'] = 'Producto has been addedd successfully';
			}else{
				$resp['msg'] = " Producto has been updated successfully.";
			}
	...
	function delete_product(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Producto successfully deleted.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function add_to_card(){
		...
		if($check > 0 ){
			$resp['status'] = 'failed';
			$resp['msg'] = "Producto already exist in your cart.";
		}else{
		...
		if($resp['status'] == 'success'){
			$this->settings->set_flashdata('success', 'Producto has been added to cart.');
		}
		return json_encode($resp);
	}
