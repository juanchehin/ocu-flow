...
Class Users extends DBConnection {
	...
	public function save_users(){
		...
		if(empty($id)){
			...
			if($qry){
				...
				$this->settings->set_flashdata('success','User Detalles successfully saved.');
				foreach($_POST as $k => $v){
					if($k != 'id'){
						if(!empty($data)) $data .=" , ";
			...
			if($qry){
				$this->settings->set_flashdata('success','User Detalles successfully updated.');
				foreach($_POST as $k => $v){
					if($k != 'id'){
						if(!empty($data)) $data .=" , ";
						if($this->settings->userdata('id') == $id)
	...
	public function delete_users(){
		...
		if($qry){
			$this->settings->set_flashdata('success','User Detalles successfully deleted.');
	...
	function registration(){
		...
		if($save){
			...
			$resp['uid'] = $uid;
			if(!empty($id))
				$resp['msg'] = 'User Detalles has been updated successfully';
			else
	...
	public function delete_customer(){
		...
		if($qry){
			$this->settings->set_flashdata('success','Customer Detalles has been deleted successfully.');
			$resp['status'] = 'success';
